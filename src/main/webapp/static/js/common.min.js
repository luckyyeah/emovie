...
function gopay() {
    ...
    i.on("dialog:action",
    function(i) {
        0 === i.index ? window.location.href = "wap/index/"+$("#CHANNEL_NO").val(): window.location.href = "wapmovie/checkPay"
    })
}
...
/*    var tab = new fz.Scroll('#header', {
    ...
    function() {
        ...
        if ("" !== iftry || ispay > 0) window.location.href = "wapmovie/videoPlay/"+$("#CHANNEL_NO").val() +"/"+i;
            ...
            o.on("dialog:action",
            function(i) {
                1 == i.index && (window.location.href = "wapmovie/checkPay")
            })
        }
        return ! 1
    }), 1 > ispay) {
...
if (/play/i.test(location.pathname)) if ($("#playerwrap").css({
    ...
    $("#playTip").html('非会员只能试看20秒，<a href="wapmovie/checkPay">成为会员</a>观看全部'),
    $("#playTip2").html('<a href="wapmovie/checkPay"><i class="ui-icon-star"></i>立即成为会员获得更多福利<i class="ui-icon-arrow"></i></a>');
...
if (/check/i.test(location.pathname)) {
    var img = ["b1.gif", "b2.gif"],paylink,
    idx = Math.floor(2 * Math.random()),
    videoimgHeight = $(".videoimg").width() / 1.97;
    debug && console.log(videoimgHeight),
    $(".videoimg").css("height", videoimgHeight).html('<img src="static/images/' + img[idx] + '" width="100%">');
