function vippop() {
    ...
    i.on("dialog:action",
    function(i) {
        0 === i.index && (window.location.href = "wapvv/index/"+$("#CHANNEL_NO").val())
    })
}
function gopay() {
    ...
    function(i) {
         ...
         window.location.href = "wapvv/checkPay?CHANNEL_NO="+CHANNEL_NO
    })
}
...
/*    var i = $.dialog({
    ...
    i.on("dialog:action",
    function(i) {
    	window.location.href ="wapvv/checkPay";
    })*/
}
function getNo() {
    var i = "x_a_no",
...
function() {
	  if(getCookie("COLUMN_ID")==null || getCookie("COLUMN_ID")==""){
		  window.location.href = "wapvv/index/"+$("#CHANNEL_NO").val()
	  } else {
		  window.location.href = "wapvv/listColumnVideo/"+$("#CHANNEL_NO").val()+"/"+getCookie("COLUMN_ID")
	  }
...
/*    var tab = new fz.Scroll('#header', {
    ...
    function() {
        ...
        if ("" !== iftry || ispay > 0) window.location.href = "wapvv/videoPlay/"+$("#CHANNEL_NO").val() +"/"+i;
        else {
            var e = '<div id="paybox"class="ui-dialog"><div class="ui-dialog-cnt"><a class="ui-icon-close-page"data-role="button"></a><div class="info"><h4>试看结束,充值VIP观看更多高清爽片</h4><p class="ui-txt-red">（只需支付一次，享受所有影片观看权限！）</p><p>VIP注意事项：<br>1.一次性支付，享有所有影片观看权限。<br><div class="payBtn"><a class="paybtn weixin"data-role="button">确定</a></div></div></div></div>';
            $("body").append(e);
            var o = $("#paybox").dialog("show");
            o.on("dialog:action",
            function(i) {
            	 var CHANNEL_NO =$("#CHANNEL_NO").val();
                1 == i.index && (window.location.href = "wapvv/checkPay?CHANNEL_NO="+CHANNEL_NO)
            })
        }
        return ! 1
    }), 1 > ispay) {
...
if (/play/i.test(location.pathname)) if ($("#playerwrap").css({
    ...
    height: $(window).height() - 300 + "px"
	 ...
	 var CHANNEL_NO =$("#CHANNEL_NO").val();
    $("#playTip").html('非会员只能试看20秒，<a href="wapvv/checkPay?CHANNEL_NO='+CHANNEL_NO+'">成为会员</a>观看全部'),
    $("#playTip2").html('<a href="wapvv/checkPay?CHANNEL_NO='+CHANNEL_NO+'"><i class="ui-icon-star"></i>立即成为会员获得更多福利<i class="ui-icon-arrow"></i></a>');
    var trymp4 = decodeURI(getCookie("trymp4")).split("|"),
    idx = Math.floor(Math.random() * trymp4.length);
    var videoId = $("#videoId").val();
    var url = "wapvv/getPlayInfo/" +videoId +"/1";
    ...
    $("#playTip2").hide();
    var videoId = $("#videoId").val();
    var url = "wapvv/getPlayInfo/" +videoId +"/2";
    $.ajax({
    	type : "post",
    	url : url,
    	data :'',
