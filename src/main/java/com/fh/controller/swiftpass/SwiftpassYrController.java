 ...
 */
@Controller
@RequestMapping(value="/thirdpayyr")
public class SwiftpassYrController extends BaseController {
	...
	public ModelAndView goPay(Page page){
		...
		try{
			...
			if("2".equals(pd.getString("version"))){
				payUrl= createOrder(map,channelNo,SwiftpassConfigYr.callback_urlv2+"/"+channelNo);
			} else if("3".equals(pd.getString("version"))){
				payUrl= createOrder(map,channelNo,SwiftpassConfigYr.callback_urlv3+"/"+channelNo);
			} else{
				payUrl= createOrder(map,channelNo,SwiftpassConfigYr.callback_url+"/"+channelNo);
			}
	...
	private String createOrder(SortedMap<String,String> map,String channelNo,String return_url  ) throws Exception{
		    String  payUrl = "";
//        map.put("mch_id", SwiftpassConfigYr.mch_id);
//        map.put("notify_url", SwiftpassConfigYr.notify_url+"/"+channelNo);
//        map.put("nonce_str", String.valueOf(new Date().getTime()));
//        map.put("callback_url",return_url);
//        map.put("body", SwiftpassConfigYr.body);
...
//	        map.put("callback_url",return_url);	        
	        ...
	        map.put("version", "1.0");		//版本号，默认即可
	        map.put("service", "pay.weixin.wappay");	//服务号，不可更改
	        
	        map.put("mch_id", SwiftpassConfigYr.mch_id);		//商户号
	        map.put("notify_url", SwiftpassConfigYr.notify_url+"/"+channelNo);		//接收回调地址，商家自行配置
       ...
       // map.put("mch_create_ip", "220.184.228.242");
        ...
        String sign = MD5.sign(preStr, "&key=" + SwiftpassConfigYr.key, "utf-8");		//生成签名
        map.put("sign", sign);
       // System.out.println("map: " + map);
        String reqUrl = SwiftpassConfigYr.req_url;
	            ...
	            if(resString != null && !"".equals(resString)){
	               ...
	               // System.out.println("通知内容：" + res);
	                if(map.containsKey("sign")){
	                    if(!SignUtils.checkParam(map, SwiftpassConfigYr.key)){
	                        res = "验证签名不通过";
	                        respString = "fail";
	                    }else{
